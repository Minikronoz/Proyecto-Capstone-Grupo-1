  ...
  try {
    ...
    while (hasNext) {
      ...
      const items = await page.$$eval("div.border.rounded-t-lg.flex a[href*='/p']", (cards) =>
        cards.map((el) => {
          try {
            let brand = el.querySelector(".text-sm.text-gray-500")?.innerText?.trim() || "";
            const title = el.querySelector(".product-card-name")?.innerText?.trim() || "";
            const price = el.querySelector(".text-lg, .font-bold")?.innerText?.trim() || "";
            const priceNormal = el.querySelector(".line-through, .text-neutral500")?.innerText?.trim() || null;
            const pricePerUnit = el.querySelector(".ppum-price-container span.text-greyDark")?.innerText?.trim() || null;
